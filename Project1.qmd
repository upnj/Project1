...
=======
>>>>>>> 4089b5b90f3c7be3365071be429f1710d6095bf4
```{r}
...
```{r}
#Geography Checker
geo_checker <- function(geography_level, geo_value) {
  geo_vars <- c("All", "Region", "Division", "State")
  if (geography_level %in% geo_vars) {
    print("Geography level is valid.") 
  }
...
```{r}
# Query the Census API
# Created helper function to use GET here

census_query <- function(year, geography_level, geo_value, get) {
    initial_parse_final <- tibble()
    year_list <- strsplit(year, ",")
    url <- "https://api.census.gov/data/"
    geo_meta <- paste0("&for=", geography_level,":",geo_value)
    ...
    for (i in year_list[[1]]) {
      query_url <- paste0(url,i,"/acs/acs1/pums?","get=PWGTP,GASP,",get,geo_meta)
      initial_response <- GET(url = query_url)
      initial_parse <- fromJSON(rawToChar(initial_response$content))
      col_names <- (initial_parse[1,])
      colnames(initial_parse) <- col_names
...
# Testing purposes/example
initial_parse <- census_query(year = "2011", "State", "08", "AGEP,SEX")
```
...
main_query <- function(year = "2022", 
                       ...
                       geo_value, 
                       get = "SEX,AGEP,PWGTP") {
    initial_parse <- census_query(year, geography_level, geo_value, get)
    initial_parse <- num_checker(initial_parse)
    initial_parse <- cat_checker(initial_parse)
    geo_checker(geography_level, geo_value)
    year_checker(year) 
    return(initial_parse)
} 
...
# Testing purposes/example
test <- main_query("2018,2022,2015", "State", "08", "JWAP,SEX,SCHL,GASP")
```
